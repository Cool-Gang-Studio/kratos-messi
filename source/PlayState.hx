...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'stadium':
				var stadium:FlxSprite = new FlxSprite(-460, -260, Paths.image('kratos_messi_bg', 'shared'));
				stadium.scale.set(1.4, 1.4);
				add(stadium);
		}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			var goKillMePlease:String = Paths.formatToSongPath(PlayState.SONG.song);
			switch (goKillMePlease)
			{	
				case 'kratos-messi':
					if (health > 0.2)
					{
